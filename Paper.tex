...
The SASA of an atom with radius $\varrho_i$ lies on a sphere where $K=\frac{1}{\varrho_i^2}$. We can divide this SASA into many polygons bounded by the circles of intersection with neighboring spheres (See Figure 1). If $M$ is one particular polygon, we can write 
\begin{align}\nonumber
...
\begin{figure}[h!]
\caption{A power diagram comprised of power cells (left) and its Delaunay Triangulation (right).}
...
\begin{figure}[h!]
\caption{The dual complex (K) of the Delaunay Triangulation only includes those power cells that non-empty intersection.}
...
\begin{figure}[h!]
\caption{The star of K and link of the star are shown.}
...
\section{Application of ALPHAMOL to SASA Calculations}
